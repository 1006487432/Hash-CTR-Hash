...
from wideblock.algorithms.xcbv1 import xcb_aes_v1_decrypt, xcb_aes_v1_encrypt
...
def _sample_bytes(label: str, length: int) -> bytes:
    ...
    while len(out) < length:
        seed = f"{label}:{counter}".encode("ascii")
...
ROUNDTRIP_CASES = [
    ...
    RoundTripCase("case8", _sample_bytes("k8", 16), _sample_bytes("p8", 255), _sample_bytes("a8", 0)),
    RoundTripCase("case9", _sample_bytes("k9", 16), _sample_bytes("p9", 256), _sample_bytes("a9", 33)),
    RoundTripCase("case10", _sample_bytes("k10", 16), _sample_bytes("p10", 512), _sample_bytes("a10", 16)),
]
...
def main() -> int:
    return run_roundtrip_cases(
        ...
        encrypt_fn=xcb_aes_v1_encrypt,
        decrypt_fn=xcb_aes_v1_decrypt,
    )
